...
class ViT(nn.Module):
    def __init__(self, *, num_patches, patch_dim, num_classes, dim, depth, heads, mlp_dim, pool = 'cls', channels = 1, dim_head = 64, dropout = 0., emb_dropout = 0.):
        super().__init__()
        ...
        self.mlp_head = nn.Sequential(
            ...
            # For softmax with 2 classes
            # nn.Linear(dim, num_classes)
            
            # For sigmoid with 1 class
            nn.Linear(dim, 1)
    ...
    def forward(self, features):
        ...
        #print("VIT - cls_tokens", cls_tokens.shape)

        x = torch.cat((cls_tokens, x), dim=1)
        
        #print("VIT - pos_embedding", self.pos_embedding[:, :(n + 1)].shape)
